...
programs as a control flow graph, and performs simple operations like dead code
elimination and constant folding, before finally generating assembly code for a
simple abstract machine. The passes are listed
in~\figref{fig:coursecompilerpasses}, the input grammar of the compiler is given
in~\figref{fig:coursecompilergrammar}, and an example of the compiler's behavior
is shown in~\figref{fig:coursecompilerexample}.

\mav{Make figures explaining the course compiler.}
...
pointer-only modes, expressed as slowdowns of the pointer-based approaches over
...
%
Nevertheless the performance of pointer-based approaches is good at small sizes:
(1) trees are small and fit in cache, (2) the single-threaded workload can
acquire all of the last level cache, not contending with other threads on the
16-core machine. The end result is that the system is able to mask the bad
...
\begin{figure}
  ...
  \caption{Speedup of serialized implementation of Racket core fold benchmark versus
    pointer-based implementation}\label{fig:racketcore-map}
\end{figure}
...
We use the following set of of \numbenchmarks \csk{udpate this number before submission.} benchmarks to evaluate performance.
%
