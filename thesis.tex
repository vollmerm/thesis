...
buffer-processing code as in the C++ code shown in \figref{fig:cpp-example}.
...
(\il{ptr}, in \figref{fig:cpp-example}). Cursors need to be manipulated carefully to visit
...
\subsection{Compiler (1/4): Finding Traversals}
...
\subsection{Compiler (2/4): Implementing Random Access}
...
only backtrack at most once.
%% Thus this back-track can happen {\em at most once}\footnote{The marked set of constructors
...
%%   (a : @\tyatlocreg{Tree}{l_a}{r_1}@) (b : @\tyatlocreg{Tree}{l_b}{r_1}@) -> rightmost [@\locreg{l_{b}}{r_1}@] b
%% \end{code}
...
datatypes, then it must use the same approach we previously used in Gibbon~\cite{ecoop17-gibbon}:
