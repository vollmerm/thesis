...
\mav{Copy over some of the high level ideas from the appendix, and maybe remove
  the appendix entirely, since it was already published in a tech report.}

\section{Extensions}\label{sec:extensions}

\mav{Talk about extensions we made to LoCal, like offsets and indirections.}
\mav{Add other junk here that doesn't fit in other sections.}
...
over serialized data (with some potential indirection). This data representation
...
serialized data must often be read serially. To change that, first, enough {\em
indexing} data must be left in the representation in order for parallel tasks to
...
computation. In this strategy, follows function: data representation is
...
We implemented this strategy in Gibbon and found that it performed as well or
better than parallel GHC and parallel MLton on a variety of benchmarks. When
utilizing 18 cores, our geomean speedup is $1.87\times$ and $3.16\times$ over
parallel MLton and GHC, respectively.
...
%
Consider the \il{buildtree} program, which creates a binary tree of a
given size \il{n} in a given region \il{r}.
%
\mav{reference buildTree code here}
% 
...
\mav{Insert brief overview of parallel local semantics}

An evaluation of the implementation of parallel \ourcalc is given in
\secref{subsec:parallelbench}.
